...
class Transport (threading.Thread):
    ...
    def open_forwarded_tcpip_channel(self, src, dest):
        ...
        @param src: tuple of originator's address and port
        @param dest: tuple of local (server) connected address and port
        """
    ...
    def open_channel(self, kind, dest_addr=None, src_addr=None):
        ...
        try:
            ...
            m = Message()
            m.add_byte(bchr(MSG_CHANNEL_OPEN))
            m.add_string(kind)
            m.add_int(chanid)
    ...
    def send_ignore(self, bytes=None):
        ...
        """
        m = Message()
        m.add_byte(bchr(MSG_IGNORE))
        if bytes is None:
            bytes = (bord(rng.read(1)[0]) % 32) + 10
    ...
    def global_request(self, kind, data=None, wait=True):
        ...
        if wait:
            self.completion_event = threading.Event()
        m = Message()
        m.add_byte(bchr(MSG_GLOBAL_REQUEST))
        m.add_string(kind)
        m.add_boolean(wait)
        if data is not None:
            m.add(*data)
        ...
        try:
            ...
            while self.active:
                ...
                if ptype in self._handler_table:
                    ...
                    msg = Message()
                    msg.add_byte(bchr(MSG_UNIMPLEMENTED))
                    msg.add_int(m.seqno)
    ...
    def _parse_kex_init(self, m):
        ...
        self.remote_kex_init = bchr(MSG_KEXINIT) + m.get_so_far()
    ...
    def _parse_global_request(self, m):
        ...
        if want_reply:
            msg = Message()
            if ok:
                msg.add_byte(bchr(MSG_REQUEST_SUCCESS))
                msg.add(*extra)
            else:
                msg.add_byte(bchr(MSG_REQUEST_FAILURE))
            self._send_message(msg)
	...
	if (kind == 'auth-agent@openssh.com') and (self._forward_agent_handler is not None):
        ...
        if reject:
            msg = Message()
            msg.add_byte(bchr(MSG_CHANNEL_OPEN_FAILURE))
            msg.add_int(chanid)
            msg.add_int(reason)
            msg.add_string('')
            msg.add_string('en')
        ...
        try:
            ...
            self.lock.release()
        m = Message()
        m.add_byte(bchr(MSG_CHANNEL_OPEN_SUCCESS))
        m.add_int(chanid)
        m.add_int(my_chanid)
        m.add_int(self.window_size)
        m.add_int(self.max_packet_size)
